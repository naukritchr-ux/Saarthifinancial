                    ...
                    if bill_date:
                        ...
                        if days_old >= 30:
                            ...
                            outstanding_receivables.append({
                                ...
                                'priority': priority,
                                'reason': f"Unpaid balance of Rs. {service_charges - amount_received:.2f} outstanding for {days_old} days"
                            })
...
def get_active_predictions():
    ...
    try:
        ...
        # Query all active/inprogress enquiries excluding duplicates, including franchisee and TL fields
        ...
        for r in active_rows:
            ...
            # Feature extraction
            # 1. Industry — prefer the real column (from master_final_pipeline
            # via import_master_final.py); fall back to the keyword guess only
            # for rows imported before that column existed or left blank.
            if r['industry']:
                industry = r['industry']
            else:
            ...
            # Missing for clients acquired before dateClientAcquired was tracked;
            # falls back to 0 rather than crashing (matches the median-fill
            # train_models.py uses only in the sense that both are safe defaults —
            # 0 is intentionally conservative here, not the training median,
            # since we have no per-request access to the training set's median).
            date_acquired = r['dateClientAcquired']
            ...
            if models_loaded:
                try:
                    ...
                    def safe_transform(encoder, val):
                        ...
                        # train_models.py buckets rare industry values (<10
                        # occurrences) into "Other" before encoding — a live
                        # value the encoder never saw is exactly that case,
                        # so prefer that bucket over a generic "Unknown".
