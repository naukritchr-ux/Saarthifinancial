...
import { normalizeFY, deriveFYFromDate, makeKey } from '../utils/fyHelper.js';
...
export const syncSaarthiLiveApi = async (req, res) => {
  try {
    ...
    // STEP 1 diagnostic — run a live sync once with this uncommented to confirm
    // the real FY field name the CRM uses, then remove it again:
    // console.log('DEBUG invoicesData[0]:', JSON.stringify(invoicesData[0], null, 2));
    // console.log('DEBUG clientsData[0]:', JSON.stringify(clientsData[0], null, 2));
    // console.log('DEBUG legalsData[0]:', JSON.stringify(legalsData[0], null, 2));

    // Pull whatever FY-like field the CRM sends off an invoice. We don't know
    // the exact key name yet (see STEP 1 above), so try every plausible
    // candidate before falling back to deriving it from an invoice/voucher
    // date. Returns a canonical "FY YYYY-YY" string, or null if genuinely
    // undeterminable — never fabricates a default.
    const extractInvoiceFY = (inv) => {
      const candidateFields = [
        inv.financialYear, inv.finYear, inv.fin_year, inv.FY, inv.fy,
        inv.financial_year, inv.finYr, inv.fin_yr
      ];
      for (const c of candidateFields) {
        const norm = normalizeFY(c);
        if (norm) return norm;
      }
      // Fall back to deriving FY from an invoice/voucher date if the CRM
      // doesn't send an explicit FY field.
      const dateCandidates = [
        inv.invoiceDate, inv.invoice_date, inv.voucherDate, inv.voucher_date,
        inv.billDate, inv.bill_date, inv.date, inv.createdAt, inv.created_at
      ];
      for (const d of dateCandidates) {
        const derived = deriveFYFromDate(d);
        if (derived) return derived;
      }
      return null;
    };

    // Accumulate invoice TDS amounts from CRM, keyed by (tan|fy) / (norm|fy)
    // instead of a flat total — every invoice used to get summed into one
    // running total per TAN regardless of year, which destroyed the year
    // breakdown before it was ever stored.
    const crmTdsByTan = new Map();
    const crmTdsByNormName = new Map();
    // Which financial years we actually saw invoice activity in, per
    // tan/normalized-name — used below to know how many per-year
    // clientMasters rows a given client needs.
    const tanFYs = new Map();
    const normFYs = new Map();
    ...
    invoicesData.forEach(inv => {
      ...
      if (tds > 0) {
        ...
        const fy = extractInvoiceFY(inv);

        if (tan) {
          const key = makeKey(tan, fy);
          crmTdsByTan.set(key, (crmTdsByTan.get(key) || 0) + tds);
          if (!tanFYs.has(tan)) tanFYs.set(tan, new Set());
          tanFYs.get(tan).add(fy || null);
        }
        if (norm) {
          const key = makeKey(norm, fy);
          crmTdsByNormName.set(key, (crmTdsByNormName.get(key) || 0) + tds);
          if (!normFYs.has(norm)) normFYs.set(norm, new Set());
          normFYs.get(norm).add(fy || null);
        }
      }
    });
    ...
    // A client is no longer "one entry per client" — it's "one entry per
    // (client, financial_year)" wherever invoice TDS data exists, same
    // shape as tds_26as_entries/tds_tally_entries. `baseFields` holds
    // everything with no year dimension (contact info etc.), which gets
    // copied identically onto each year-row; `itemTan`/`norm` are used to
    // look up which years actually have invoice activity and what the
    // per-year TDS total was.
    const pushYearRows = (baseFields, itemTan, norm, fallbackTds) => {
      const fySet = (itemTan && tanFYs.get(itemTan)) || (norm && normFYs.get(norm)) || null;
      if (fySet && fySet.size > 0) {
        for (const fy of fySet) {
          const tds = (itemTan && crmTdsByTan.get(makeKey(itemTan, fy))) ||
            (norm && crmTdsByNormName.get(makeKey(norm, fy))) || 0;
          clientMasters.push({ ...baseFields, financial_year: fy || null, saarthi_tds: tds });
        }
      } else {
        // No invoice-year data matched this client at all — one row, no
        // fabricated financial year (this used to default to 'FY 2025-26').
        clientMasters.push({ ...baseFields, financial_year: null, saarthi_tds: fallbackTds });
      }
    };
    ...
    clientsData.forEach(item => {
      ...
      const fallbackTds = parseFloat(item.tdsAmount || item.tds_amount || item.tds || 0) || 0;
      ...
      pushYearRows(baseFields, itemTan && tanRegex.test(itemTan) ? itemTan : null, norm, fallbackTds);
    });
    ...
    legalsData.forEach(item => {
      ...
      pushYearRows(baseFields, tanRegex.test(tan) ? tan : null, norm, fallbackTds);
    });
    ...
    // Matching is now on (identity, financial_year) composite — matching on
    // identity alone would keep updating a single row per client and the
    // year split would collapse right back on the very next sync.
    ...
    (existingAll || []).forEach(row => {
      if (row.saarthi_client_id) clientIdMap.set(makeKey(row.saarthi_client_id, row.financial_year), row.id);
      if (row.tan_no) tanMap.set(makeKey(row.tan_no, row.financial_year), row.id);
      if (row.pan_no) panMap.set(makeKey(row.pan_no, row.financial_year), row.id);
      if (row.company_name) nameMap.set(makeKey(row.company_name, row.financial_year), row.id);
    });
    ...
    for (const master of usableMasters) {
      let matchedId = null;
      if (master.saarthi_client_id && clientIdMap.has(makeKey(master.saarthi_client_id, master.financial_year))) {
        matchedId = clientIdMap.get(makeKey(master.saarthi_client_id, master.financial_year));
      } else if (master.tan_no && tanMap.has(makeKey(master.tan_no, master.financial_year))) {
        matchedId = tanMap.get(makeKey(master.tan_no, master.financial_year));
      } else if (master.pan_no && panMap.has(makeKey(master.pan_no, master.financial_year))) {
        matchedId = panMap.get(makeKey(master.pan_no, master.financial_year));
      } else if (master.company_name && nameMap.has(makeKey(master.company_name, master.financial_year))) {
        matchedId = nameMap.get(makeKey(master.company_name, master.financial_year));
      }
      ...
      if (matchedId) {
        ...
        const dedupeKey = master.saarthi_client_id ? makeKey(`id_${master.saarthi_client_id}`, master.financial_year) :
          master.tan_no ? makeKey(`tan_${master.tan_no}`, master.financial_year) :
            master.pan_no ? makeKey(`pan_${master.pan_no}`, master.financial_year) :
              master.company_name ? makeKey(`name_${master.company_name}`, master.financial_year) : null;
      ...
      await Promise.all(chunk.map(item =>
        db.execute(`
          ...
          SET 
            ...
            financial_year = COALESCE(NULLIF(financial_year, ''), ?),
    ...
    for (let i = 0; i < toInsert.length; i += INSERT_CHUNK_SIZE) {
      ...
      for (const m of chunk) {
        ...
        params.push(
          ...
          m.email_id || null,
          m.teamleader || null,
          m.financial_year || null,   // ← real per-invoice FY, or null if undeterminable (no fabricated default)
          m.saarthi_tds || 0   // ← Saarthi Books TDS from CRM legals, scoped to this row's financial_year
        );
      }
      const sql = `
